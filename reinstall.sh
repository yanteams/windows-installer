...
translate_vi() {
    ...
    "Can not get location.")
        echo "Không lấy được thông tin vị trí."
        ;;
    File\ not\ found:*)
        echo "Không tìm thấy tệp: ${1#*: }"
        ;;
    "Not Supported OS in Container."*)
        echo "Hệ điều hành không hỗ trợ trong container. Vui lòng dùng https://github.com/LloydAsp/OsMutation"
        ;;
    *"because the CPU does not support x86-64-v3.")
        echo "Không thể cài đặt vì CPU không hỗ trợ x86-64-v3."
        ;;
    --image-name\ wrong:*)
        echo "Tham số --image-name không hợp lệ: ${1#*: }"
        ;;
    "Edition is not set.")
        echo "Chưa đặt phiên bản (edition)."
        ;;
    "Not support find this iso. Check if --image-name is wrong. Or set --iso manually.")
        echo "Không tìm được ISO tự động. Kiểm tra --image-name hoặc đặt --iso thủ công."
        ;;
    "No ARM iso for this Windows Version or Edition.")
        echo "Không có ISO ARM cho phiên bản/edition Windows này."
        ;;
    "Could not find iso for this windows edition or language.")
        echo "Không tìm thấy ISO cho edition hoặc ngôn ngữ Windows đã chọn."
        ;;
    Debian\ *\ ELTS\ does\ not\ support\ aarch64.)
        echo "Debian ELTS bản này không hỗ trợ aarch64."
        ;;
    Minimal\ cloud\ image\ is\ not\ available\ for\ *)
        echo "Không có cloud image tối giản cho phiên bản/kiến trúc này."
        ;;
    "ISO Link is empty.")
        echo "Liên kết ISO trống."
        ;;
    "All mirror failed.")
        echo "Tất cả mirror tải ISO đều thất bại."
        ;;
    "dd need --img")
        echo "Chế độ dd cần tham số --img."
        ;;
    "redhat need --img")
        echo "Cài Red Hat cần tham số --img."
        ;;
    "Install Windows need --image-name.")
        echo "Cài Windows cần tham số --image-name."
        ;;
    "not support ssh key for "*)
        echo "Không hỗ trợ ssh key cho bản phân phối này."
        ;;
    "Which GNU/Linux do not have iconv built-in?")
        echo "Bản GNU/Linux này không có iconv tích hợp?"
        ;;
    "Can't find compatible package manager. Please manually install "*)
        echo "Không tìm thấy trình quản lý gói phù hợp. Hãy tự cài: ${1##*install }"
        ;;
    ...
    "Could not generate sha512 password.")
        echo "Không tạo được mật khẩu sha512."
        ;;
    OS\ across\ *\ disk:*)
        echo "Hệ điều hành nằm trên nhiều đĩa: ${1#*: }"
        ;;
    Disk\ ID\ is\ invalid:*)
        echo "Disk ID không hợp lệ: ${1#*: }"
        ;;
    "Can not get IP info.")
        echo "Không lấy được thông tin IP."
        ;;
    "Can't find efi partition.")
        echo "Không tìm thấy phân vùng EFI."
        ;;
    "find no/multi apks.")
        echo "Không tìm thấy hoặc tìm thấy nhiều gói apk."
        ;;
    Find\ *\ *)
        echo "Tìm thấy: ${1#Find }"
        ;;
    "Can't use cloud kernel. And not enough RAM to run normal kernel.")
        echo "Không thể dùng cloud kernel và RAM không đủ cho kernel thường."
        ;;
    "Can't run this script in Live OS.")
        echo "Không thể chạy script trong Live OS."
        ;;
    ...
    "Please run as root.")
        ...
        ;;
    Invalid\ *\ value:\ *)
        echo "Giá trị không hợp lệ cho ${1#Invalid }"
        ;;
    Need\ value\ for\ *)
        echo "Thiếu giá trị cho ${1#*for }"
        ;;
    "Can't get frpc config from "*)
        echo "Không lấy được cấu hình frpc từ: ${1#*from }"
        ;;
    File\ not\ exists:*)
        echo "Tệp không tồn tại: ${1#*: }"
        ;;
    Need\ a\ username\ for\ *)
        echo "Cần username cho ${1#*for }"
        ;;
    "Can't get ssh key from "*)
        echo "Không lấy được ssh key từ: ${1#*from }"
        ;;
    SSH\ Key/File/Url\ \"*\"\ is\ invalid.)
        echo "SSH Key/File/Url không hợp lệ: ${1#*\" }"
        ;;
    Not\ a\ inf\ or\ dir:\ *)
        echo "Không phải file INF hoặc thư mục: ${1#*: }"
        ;;
    "Can't find inf files in "*)
        echo "Không tìm thấy file INF trong: ${1#*in }"
        ;;
    "Please disable secure boot first.")
        echo "Vui lòng tắt Secure Boot trước."
        ;;
    Unsupported\ arch:*)
        echo "Kiến trúc không được hỗ trợ: ${1#*: }"
        ;;
    "grub not found")
